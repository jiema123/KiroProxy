...
async def start_callback_server() -> Tuple[bool, dict]:
    """启动本地回调服务器"""
    global _callback_result, _callback_event, _callback_runner, _callback_site

    if _callback_runner is not None:
    ...
    async def handle_callback(reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        first_line = raw_request.decode("utf-8", errors="ignore").splitlines()[0] if raw_request else ""
        parts = first_line.split()
        target = parts[1] if len(parts) >= 2 else ""
        parsed = urlparse(target)
        query = parse_qs(parsed.query)

        if parsed.path != SOCIAL_AUTH_CALLBACK_PATH:
        ...
        if _callback_event is not None and _callback_event.is_set():
            writer.write(
    ...
    try:
        ...
        _callback_runner = server
        _callback_site = server
        print(f"[SocialAuth] 回调服务器已启动: {SOCIAL_AUTH_REDIRECT_URI}")
        return True, {
            "port": SOCIAL_AUTH_CALLBACK_PORT,
            "callback_url": SOCIAL_AUTH_REDIRECT_URI,
        }
    except Exception as e:
...
async def stop_callback_server():
    """停止本地回调服务器"""
    global _callback_runner, _callback_site, _callback_result, _callback_event

    if _callback_site is not None:
        _callback_site.close()
        await _callback_site.wait_closed()
        _callback_site = None
...
def get_callback_result() -> Optional[dict]:
    """非阻塞读取回调结果"""
    if _callback_event is None or not _callback_event.is_set():
        return None
